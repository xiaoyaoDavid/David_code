...
// D:\website/coop/trunk/src/bz/project/ylxsd/xsd.js
/**
 *  全局函数处理
 *  -----------------------------
 *  作者：叼怎么写！- -||
 ...
 */
  ...
  page_start    : function(){
    ...
    car2._pageBtn.on('touchstart',car2.page_click_start);
  ...
  page_click_start : function(e){
      car2._moveInit = true;
      car2._handleEvent('start');
      car2._touchStartValY = window.event.pageY;
      //car2._touchDeltaY = window.event.pageY;
      moveP = window.event.pageY-100;
      var $self = car2._page.eq(car2._pageNow);
      click_node = car2.page_position(e,moveP,$self);
      car2.page_touch_end();
      car2.page_translate(click_node);
  ...
  page_translate  : function(node){
    ...
    var y_1,y_2,scale,
      y = car2._touchDeltaY,
      _translateZ = car2._translateZ();

    // 切换的页面移动
    if($(node[0]).attr('data-translate')) {
      if(e.type=="touchstart"){
        y_1 = y + parseInt($(node[0]).attr('data-translate'))-$(window).height()+100;
      }else{
        y_1 = y + parseInt($(node[0]).attr('data-translate'));
      }
    }
    console.log(y_1);
    node[0].style[car2._prefixStyle('transform')] = 'translate(0,'+y_1+'px)';
    ...
    y_2 = y_2/5;
    node[1].style[car2._prefixStyle('transform')] = 'translate(0,'+y_2+'px)'+_translateZ;
...
//alert(_translateZ)
  ...
  // page_click_start : function(){
  //        car2._moveInit = true;
  //        car2._handleEvent('start');
  //     // // // alert(car2._pageNow)
  //     //   car2._page.eq(car2._pageNow).addClass('f-hide');
  //     //   car2._page.eq(car2._pageNow+1).removeClass('f-hide');
  //     //   car2._pageNow++;

    
  //   car2._page.removeClass('f-hide').css("top",$(window).height());
  //   car2._page.eq(0).css("top",0);
  //   var now = car2._page.eq(car2._pageNow)[0],
  //       next = car2._page.eq(car2._pageNext)[0],
  //       node = [next, now],
  //       _translateZ = car2._translateZ(),
  //       y_1 = y_2 = y = -$(now).height();
    
  //     // node[0].style[car2._prefixStyle('transform')] = 'translate(0,'+y_1+'px)';
  //     // node[0].style[car2._prefixStyle('transition')] = 'all .3s';
  //     car2._page.eq(car2._pageNow).css({"-webkit-transition":"all .3s","top":-667})
  //     // 当前的页面移动
  //     //y_2 = y_2/5;
    
  //     node[1]=car2._page.eq(car2._pageNow+1);
  //     console.log(node[1])
  //     node[1].css({"-webkit-transition":"all .3s","top":0})
  //     // node[1].style[car2._prefixStyle('transform')] = 'translate(0,'+y_2+'px)'+_translateZ;
  //     // node[1].style[car2._prefixStyle('transition')] = 'all .3s';
  //     car2._page.eq(car2._pageNow).css("top",0)
  //     car2._page.eq(car2._pageNow+1).css("top",0)
  //     car2._pageNow++;
  //           if(car2._pageNow>=6){
  //        car2._pageNow==0;
  //        $('.u-arrow').addClass('f-hide');
  //     }
  //     if (car2._pageNow == 1 && !car2.isScrolleInit) {
  //         $('#j-scroller').find('li').length > 1 && scrool.init();
  //         car2.isScrolleInit = true;
  //     }
  // },
 ...
 */
  ...
  //iphone获取视频播放地址
  authSucc : function(json) {
    if (json.statusCode != 1001 || json.playStatus !=0 ) return;
    var movieVO = this.movieVO = {}, data, vtype;
        for (var i=0; i<json.data.length; i++) {
            data = json.data[i].infos;
        ...
        var defi = 1;
        if (movieVO['1']) defi = 1;
        else if (movieVO['2']) defi = 2;

        var src = movieVO[defi].mainUrl,
          tpl = '<div><video autoplay src="'+src+'" style="width:100%;height:100%;"></video></div><div class="big_play"></div>';
        this.target.parent().append(tpl);

        // location.href = movieVO[defi].mainUrl;
  },
 ...
 */
  ...
  lazy_img : function(){
    ...
    lazyNode.each(function(){
      var self = $(this);
      if(self.is('img')){
        self.attr('src','http://i2.letvimg.com/img/201407/03/load.gif');
      }else{
        // 把原来的图片预先保存下来
        ...
        self.css({
          'background-image'  : 'url(http://i2.letvimg.com/img/201407/03/load.gif)',
 ...
 */
  ...
  height_auto : function(ele,val){
    ele.children('.page-con').css('height',900);
 ...
 */
  ...
  plugin : function(){
    ...
    // 音符飘逸
    $('#coffee_flow').coffee({
      steams        : ["<img src='http://i0.letvimg.com/img/201407/03/1456/audio_widget_01@2x.png' />","<img src='http://i0.letvimg.com/img/201407/03/1456/audio_widget_01@2x.png' />"], 
      steamHeight     : 100,
      steamWidth      : 44 
    });
    ...
    var node = $('#j-mengban')[0],
      url = 'http://i0.letvimg.com/img/201407/03/img0.jpg',
 ...
 */
  ...
  //图片加载
  loadAllImg : function() {
    var elems = $('img[img-src]'),
      total = elems.length,
      count = 0;

    elems.each(function() {
      var dom = $(this);
      if (dom.attr('src')) return;

      this.onload = this.onerror = this.onabord = function() {
        if (++count>=total) {
          car2.show();
        }
      }
      dom.attr('src', dom.attr('img-src'));
    });
  },

  divImgPreload : function () {
    var elems = $('.lazy-img'), item, src, img, count = 0;
